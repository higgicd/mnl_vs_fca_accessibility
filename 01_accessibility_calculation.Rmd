...
Hamiltontimes <- ttm %>% 
  ...
  rename(DAUID = fromId) %>%
  semi_join(data_da_2016_poly %>% transmute(DAUID = GeoUID, Households) %>% filter(Households > 0))

#population <- read.csv("./data/DA Populations.csv") # unused
...
Ham_households <- data_da_2016_poly %>% 
  st_drop_geometry() %>% 
  transmute(DAUID = GeoUID, Data = Households)
```
...
data_da_2016_poly %<>% 
  left_join(ai %>% 
              transmute(GeoUID = as.character(DAUID), mnl_access = Value), 
            by = "GeoUID")

doctors %<>% left_join(Rj_new %>% transmute(doctor_id = OBJECTID, mnl_dcr = Ratios), by = "doctor_id")
```
